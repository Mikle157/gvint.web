const BRACKET = {
    ...
    executeSimulation(match, tournament, r, m) {
        ...
        // Нараховуємо переможцю +15 MMR, а програвшому знімаємо -10 MMR
        this.updatePlayerStatsAfterMatch(match.player1, match.player2, match.winnerId);

        // Advance winner to next round
        this.advanceWinner(tournament, r, m, match.winnerId);
    },

    // Helper to dynamically update player MMR and wins after simulations or matches
    updatePlayerStatsAfterMatch(p1, p2, winnerId) {
        const winner = (p1.id === winnerId) ? p1 : p2;
        const loser = (p1.id === winnerId) ? p2 : p1;

        // Update values
        winner.wins = (winner.wins || 0) + 1;
        winner.mmr = (winner.mmr || 1000) + 15;
        loser.mmr = Math.max(100, (loser.mmr || 1000) - 10);

        // If the current user is one of the players, sync their local session immediately
        if (window.APP && window.APP.currentUser) {
            if (winner.id === window.APP.currentUser.id) {
                window.APP.currentUser.wins = winner.wins;
                window.APP.currentUser.mmr = winner.mmr;
            } else if (loser.id === window.APP.currentUser.id) {
                window.APP.currentUser.mmr = loser.mmr;
            }
            
            // Re-render UI views so players see changed stats right away
            if (typeof window.APP.renderProfile === 'function') {
                window.APP.renderProfile();
            }
            if (typeof window.APP.renderLeaderboard === 'function') {
                window.APP.renderLeaderboard();
            }

            // Sync with Firebase RTDB (no-cost Realtime Database updates)
            if (typeof window.APP.syncUserStats === 'function') {
                window.APP.syncUserStats(p1.id, p1.mmr, p1.wins || 0);
                window.APP.syncUserStats(p2.id, p2.mmr, p2.wins || 0);
            }
        }
    },
